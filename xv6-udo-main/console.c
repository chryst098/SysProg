...
static int my_crt_snapshot_valid=0;
static int my_crt_snapshot_pos=0;
static ushort my_crt_snapshot_text_n_attr[2000]={0};

static int cgagetcursor(){
    int pos;
...
static void cgasetcursor(int pos){
...
static void cgaputc(int c) {
    ...
    if(currentvgamode==0x03){//if text mode, read hardware cursor
        pos = cgagetcursor(); // Cursor position: col + 80*row.
        crt = VGA_0x03_MEMORY;
    } else{//graphics mode, use snapshot 
        pos = my_crt_snapshot_pos;
        crt = my_crt_snapshot_text_n_attr;
    }
    ...
    if(currentvgamode==0x03)//if text mode update hardware cursor
        cgasetcursor(pos);
    else//graphic mode, update snapshot cursor
        my_crt_snapshot_pos=pos;
}
...
int consolevgamode(int vgamode) {
    ...
    switch (vgamode)
    {
        case 0x03: {
            ...
            if(my_crt_snapshot_valid==1){//if valid, restore snapshot
                cgasetcursor(my_crt_snapshot_pos);
                memmove(VGA_0x03_MEMORY, my_crt_snapshot_text_n_attr, 2000*sizeof(*my_crt_snapshot_text_n_attr));
            }
        } break;

        case 0x12: {
            if(currentvgamode==0x03){//if from text, take the snapshot
                my_crt_snapshot_valid=1;
                my_crt_snapshot_pos=cgagetcursor();
                memmove(my_crt_snapshot_text_n_attr, VGA_0x03_MEMORY, 2000*sizeof(*my_crt_snapshot_text_n_attr));
            }
